(function(window) {
    /*** Client Config Start ***/
    const CHANGELOG_TEXT = `Last client update: Oct 3, 2022\n\n* See github.com/ABCxFF/diepcustom for information`
    const MODES = [
        "FFA",
        "*",
        "*",
    ...
    const CHANGELOG = new Uint8Array(140);
    CHANGELOG.set(new TextEncoder().encode(CHANGELOG_TEXT));
    const Module = {};
    var arguments = [];
    var cp5 = {
        contexts: [],
    ...
    function createWasm() {
        ...
        function instantiateArrayBuffer(receiver) {
            return getBinaryPromise().then(function(binary) {
                ...
                // Since survival.length === 8 and maze.length === 4, they are shortened into i64.const and i32.const instead of loaded from memory
                const kGM2 = {
                    "Last updated: January 19th 2020\n\n* Nothing. But we'd like to invite you to try out our\n  new game that's under active development: florr.io\n": CHANGELOG,
                }
        
                binary = new Uint8Array(binary);

                for (const v in kGM2) {
                    const c = new TextEncoder().encode(v);
                    const r = new Uint8Array(c.length);
                    r.set(kGM2[v]);
                    m: for (let d = 0; d < binary.length - 20 && d >= 0; ++d) {
                        for (let j = 0; j < c.length; ++j) {
                            if (binary[d + j] !== c[j]) {
                                d = binary.indexOf(c[0], d + 2) - 1
                                continue m;
                            }
                        }
                        binary.set(r, d);
    ...
    var ASM_CONSTS = {
        ...
        8472: function() {
            if (!window.localStorage["playwireAB"])
                window.localStorage["playwireAB"] = Math.random();
        ...
        26281: function() {
            return curScript != null ? allocateUTF8("https://static.diep.io/build_6f59094d60f98fafc14371671d3ff31ef4d75d9e.wasm.js") : 0
        },
        26345: function() {
        ...
        27945: function() {
            return function() {}
            .prototype.constructor.toString.apply(document.querySelector).indexOf("return") != -1
        },
        28057: function() {
        ...
        29370: function() {
            return allocateUTF8(function() {}
            .prototype.constructor.toString.apply(CanvasRenderingContext2D.prototype.getImageData))
        },
        29518: function() {
        ...
        30225: function($0) {
            if (window["setLoadingStatus"])
                window["setLoadingStatus"](UTF8ToString($0))
        },
        30352: function(requestID, endpoint) {
            // editted
            const $id = allocateUTF8(UTF8ToString(endpoint));
            const $ipv4 = allocateUTF8(UTF8ToString(endpoint));
            const $ipv6 = allocateUTF8(UTF8ToString(endpoint));
        ...
        30836: function($0) { // editted
            ...
            var ws = window.ws = new WebSocket(url);
    ...
    function _emscripten_set_main_loop(func, fps, simulateInfiniteLoop, arg, noSetTiming) {
        ...
        Browser.mainLoop.runner = function Browser_mainLoop_runner() {
            if (!found) { // This will lag yes. But not for long hopefully
                const kGM = {
                    "FFA": (MODES[0] || "*").slice(0, 3),
                    "Survival": (MODES[1] || "*").slice(0, 8),
                    "2 Teams": (MODES[2] || "*").slice(0, 7),
                    "4 Teams": (MODES[3] || "*").slice(0, 7),
                    "Domination": (MODES[4] || "*").slice(0, 10),
                    "Tag": (MODES[5] || "*").slice(0, 3),
                    "Maze": (MODES[6] || "*").slice(0, 4),
                    "Sandbox": (MODES[7] || "*").slice(0, 7)
                }
                binary = Module.HEAPU8;
        
                for (const v in kGM) {
                    const c = new TextEncoder().encode(v);
                    const r = new Uint8Array(c.length);
                    r.set(kGM[v] instanceof Uint8Array ? kGM[v] : new TextEncoder().encode(kGM[v]));
                    m: for (let d = binary.length - 100; d > 100000; --d) {
                        for (let j = 0; j < c.length; ++j) {
                            if (binary[d + j] !== c[j]) {
                                d = binary.lastIndexOf(c[0], d - 1) + 1
                                continue m;
                            }
                        }
                        if (v === "Survival") found = true;
                        binary.set(r, d);
                        break m;
                    }
                }
